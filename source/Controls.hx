...
class Controls extends FlxActionSet
{
	...
	var _fs = new FlxActionDigital(Action.FS);
	...
	public var FS(get, never):Bool;

	inline function get_FS()
		return _fs.check();
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		switch (scheme)
		{
			case Solo:
				...
				inline bindKeys(Control.FS, keysMap.get('fullscreen'));
			case Duo(true):
				...
				inline bindKeys(Control.KEY5, [SHIFT]);
				inline bindKeys(Control.FS, [F11]);
			case Duo(false):
		...
		switch (scheme)
		{
			case Solo:
				...
				bindKeys(Control.KEY5, [SHIFT]);
				bindKeys(Control.FS, [F11]);
			case Duo(true):
